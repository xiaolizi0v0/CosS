...
蓝图使用说明： [CosS 蓝图工作区使用指南](./文档/CosS%20蓝图工作区使用指南.md)
...
- 提交 PR 时：执行蓝图工作区专属语法与 Playwright 校验，并行打包各平台产物，作为 workflow artifacts 保存（不发布 Release）
- 推送 `v*` 标签时：执行同样的蓝图模块校验与多平台打包，并发布 GitHub Release Assets
